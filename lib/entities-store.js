...
const createEntitiesStore = (now, opt = {}) => {
	const {
		initialFeedVersion,
	} = {
	...
	const put = (id, entity, expiresAt) => {
		...
		if (expiresAt !== null) {
			...
			if(timeLeft <= 0) {
				...
				// todo: add debug hook for this, track as Prometheus metric
				return;
			}
			...
			if (timeLeft <= 2147483647) {
				...
				debug('ignoring entity with expiry too far in the future', id, entity, expiresAt, timeLeft)
				// todo: add debug hook for this, track as Prometheus metric
				return;
			}
		}
	...
	const getTimestamp = () => {
		return timestamps.length > 0
			? Math.max(timestamps[timestamps.length - 1], lastMod) // highest
			: lastMod
	}

	// > String that matches the `feed_info.feed_version` from the GTFS [Schedule] feed that the realtime data is based on. Consumers can use this to identify which GTFS feed is currently active or when a new one is available to download.
	// https://gtfs.org/documentation/realtime/reference/#message-feedheader
	// see also https://gtfs.org/documentation/realtime/reference/#message-feedheader
	// see also https://github.com/google/transit/pull/434
	let feedVersion = initialFeedVersion
	const setFeedVersion = (newFeedVersion) => {
		if (feedVersion === newFeedVersion) return;
